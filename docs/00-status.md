...
| └ ─ `/one-thing` skill (run + review from any teammate's machine via `gh`) | Calvin | **Built 2026-09-04 in `.claude/skills/one-thing/`. Cold-outreach first: required inputs (recipient role, category, dated why-now with a URL that stage 00 verifies verbatim). Drives the Actions runner in mode `cold`; keys stay in Actions secrets; `review.mjs` prints the send gate. Dispatch path not yet exercised by a teammate** |
