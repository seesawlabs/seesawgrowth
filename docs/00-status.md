...
| └ Referral-source posture (`14` §8) | Calvin | **2026-09-16. If Assured is a referral source rather than a customer, the hypothesis becomes a diagnostic rather than a pitch and the fork becomes a segmentation question. A founder-owned operator at 1:4 that grew organically has probably solved knowledge propagation locally — weak customer, strong judge. Named five better candidates on the thesis, all Colorado and all dated: MorningStar (Denver, 38 properties, four transactions in nine months), Generations (eight northern CO communities inherited at once), 12 Oaks, Dial, Oakmont. **None is on the `11` list yet — MorningStar in particular belongs there.** Best untapped ask is LeGasse's podcast guest list** |
| └ Peer test + vendor landscape (`15`) | Calvin | **2026-09-16. Tested the four `14` assumptions against peers in and out of market. Three hold. The fourth — nobody builds for this shape — is partially false and corrected in `14`: Therap, iCareManager, eVero and Sandata serve scattered-site residential care, and iCareManager markets to assisted living and IDD while advertising plan currency as its headline metric. Refined claim: those platforms serve the organisation, not the caregiver in the moment, so the build is a last-mile layer over a record system. Also confirms none of the six ambient vendors supports unlicensed caregivers documenting ADLs across a shift — the mismatch is the unit of work, encounter versus shift** |
