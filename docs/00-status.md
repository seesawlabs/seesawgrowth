...
| └ ─ Target evidence (stage 03b) + the LinkedIn messages | Calvin | **Built 2026-09-04. The pipeline finds the dated reason to write from the domain alone: their own news, press and blog pages (read by us, Verified, the only openers a message may use) plus a citation-resolved search and a date-filtered news index (Cited, call material). Stage 07 now writes the LinkedIn connection note (≤300 chars) and first message (300–900), validated on the pasted text with claim ids stripped; Slack posts them paste-ready. No dated Verified opener blocks the send** |
