...
| └ ─ `/one-thing` skill (run + review from any teammate's machine via `gh`) | Calvin | **Built 2026-09-04 in `.claude/skills/one-thing/`. Drives the Actions runner; keys stay in Actions secrets; `review.mjs` prints the send gate. Dispatch path not yet exercised by a teammate** |
